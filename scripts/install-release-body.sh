...
if pgrep -x Diktat >/dev/null; then
    fail 'Avslutt Diktat fra menyfeltet og kjør samme kommando igjen.'
fi
...
cleanup() {
    ...
    if [ -n "$STAGE" ]; then
        if [ "$COMMITTED" = no ] && [ -d "$STAGE/Previous.app" ]; then
            if [ -e "$INSTALL_DIR/Diktat.app" ]; then
                mv "$INSTALL_DIR/Diktat.app" "$STAGE/Failed.app"
            fi
            mv "$STAGE/Previous.app" "$INSTALL_DIR/Diktat.app"
        fi
        rm -rf "$STAGE"
    fi
...
echo "Installerer Diktat $APP_VERSION …"
...
APP="$STAGE/unpacked/Diktat.app"
[ -d "$APP" ] || fail 'Apppakken mangler Diktat.app.'
...
download_models
if pgrep -x Diktat >/dev/null; then
    fail 'Diktat ble startet under installasjonen. Avslutt appen og kjør kommandoen igjen.'
fi
if [ -e "$INSTALL_DIR/Diktat.app" ]; then
    mv "$INSTALL_DIR/Diktat.app" "$STAGE/Previous.app"
fi
mv "$APP" "$INSTALL_DIR/Diktat.app"
COMMITTED=yes
echo "Installert: $INSTALL_DIR/Diktat.app"
echo 'Første gang: gi mikrofontilgang og aktiver Diktat under Tilgjengelighet for automatisk innliming.'
if [ "${DIKTAT_NO_OPEN:-0}" != 1 ]; then
    open "$INSTALL_DIR/Diktat.app"
fi
